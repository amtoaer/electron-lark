...
function createWindow() {
    try {
        let data = fs.readFileSync(path.join(USER_HOME, '.feishu')).toString()
        windowSize = JSON.parse(data)
    } catch (err) {
        console.log(err)
    }
    width = windowSize.width || 1000
    height = windowSize.height || 770
    ...
    mainWindow.on('resize', () => {
        ...
        fs.writeFile(path.join(USER_HOME, '.feishu'), JSON.stringify(sizeData), (error) => {
            if (error) {
                console.log(error)
            }
        })
...
function appTrayInit() {
    ...
    appTryaInstance.setToolTip('Feishu');
    appTryaInstance.setContextMenu(contextMenu)
    // the codes below don't work on my machine, ignore them.
    // appTryaInstance.on('click', () => {
    //     stopBlingIcon();
